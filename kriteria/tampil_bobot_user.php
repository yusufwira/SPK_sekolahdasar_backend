...
for ($i=0; $i < sizeof($listKriteriaId) ; $i++) { 
    if ($i == 0) {
        $statement = "kb.kriteria_2 =".$listKriteriaId[$i];
    }
    else {
        $statement = $statement." or kb.kriteria_2 =".$listKriteriaId[$i];
    }
}

$sql = "SELECT k.nama_kriteria as kriteria_1,k.idkriteria as idkriteria_1, k2.nama_kriteria  as kriteria_2,k2.idkriteria  as idkriteria_2, kb.bobot FROM kriteria_bobot kb  INNER JOIN kriteria k on kb.kriteria_1 = k.idKriteria INNER JOIN kriteria k2 on kb.kriteria_2 = k2.idKriteria Where ".$statement." ORDER by k.idkriteria";
